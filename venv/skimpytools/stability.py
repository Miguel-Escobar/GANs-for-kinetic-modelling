...
class CheckStability:
    ...
    def calc_eigenvalues_recal_vmax(self):
        ...
        for j in range(len(self.parameter_set.index)):
            ...
            self.parameter_sample = {
                v.symbol: v.value for k, v in self.kmodel.parameters.items()
            }
            ...
            self.kmodel.flux_parameter_function(
                self.kmodel, self.parameter_sample, self.sym_conc_dict, self.flux_series
            )
            ...
            this_jacobian = self.kmodel.jacobian_fun(
                self.flux_series[self.kmodel.reactions],
                self.conc_series[self.kmodel.reactants],
                self.parameter_sample,
            )

            this_real_eigenvalues = sorted(
                np.real(eigenvalues(this_jacobian.todense()))
            )
    ...
    def calc_eigenvalues(self):
        ...
        for j in range(len(self.parameter_set.index)):
            ...
            self.kmodel.parameters = param_val
            self.parameter_sample = {
                v.symbol: v.value for k, v in kmodel.parameters.items()
            }
    ...
    def _load_models(self, exp_id):
        ...
        """
        path_to_kmodel = "/home/miga/Documents/GitHub/GANs-for-kinetic-modelling/venv/models/kinetic/kin_varma_curated.yml"
        path_to_tmodel = f"/home/miga/Documents/GitHub/GANs-for-kinetic-modelling/venv/models/thermo/varma_{exp_id}"
        path_to_samples = f"/home/miga/Documents/GitHub/GANs-for-kinetic-modelling/venv/models/steady_state_samples/samples_{exp_id}.csv"
    ...
    def _prepare_models(self, parameters, parameter_names, GAN=True):
        ...
        conc_series = load_concentrations(
            self.samples,
            self.tmodel,
            self.kmodel,
            concentration_scaling=self.CONCENTRATION_SCALING,
